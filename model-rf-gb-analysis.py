...
print('Model pipeline uses random under sampler and random forest.\n')
...
class ModelOverRandomDetection():
    ...
    def eval(self, num_bootstrapped_cases=30, num_transaction_checks=400):
        ...
        pos_class = clf.classes_.max()
        pos_idx = np.where(clf.classes_ == pos_class)[0][0]
        y_score = self.pipeline.predict_proba(self.X_test)[:, pos_idx]

        # test set
        sorted_indexes = np.argsort(y_score, stable=True)
        selected_indexes = sorted_indexes[-self.num_transaction_checks:]
        y_sel = self.y_test[selected_indexes]
        total_frauds = np.sum(self.y_test)
        self.percent_frauds = 100*np.sum(y_sel)/total_frauds
        ...
        y_threshold = np.min(y_score[selected_indexes])
        y_pred = (y_score >= y_threshold)
        ba_score = balanced_accuracy_score(self.y_test, y_pred)
        print(f'Balanced accuracy score on test set: '
              f'{np.round(100*ba_score, 2)}%')
        ...
        for _ in range(self.num_bootstrapped_cases):
            random_indexes = rng.choice(len(self.y_test),
                                        ...
                                        replace=False)
            y_sel = self.y_test[random_indexes]
            sens = 100*np.sum(y_sel)/total_frauds
...
class CrossValidationCheck():
    ...
    def cv_eval(self, pipeline, num_transaction_checks=400):
        ...
        # test set
        pos_class = pipeline.classes_.max()
        pos_idx = np.where(pipeline.classes_ == pos_class)[0][0]
        y_score = pipeline.predict_proba(self.X_val)[:, pos_idx]
        sorted_indexes = np.argsort(y_score, stable=True)
        selected_indexes = sorted_indexes[-self.num_transaction_checks:]
        y_sel = self.y_val[selected_indexes]
